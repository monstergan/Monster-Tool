 ...
 */
public class MonsterRequestMappingHandlerMapping extends RequestMappingHandlerMapping {
    ...
    private RequestMappingInfo getApiVersionMappingInfo(Method method, Class<?> handlerType) {
        ...
        // 如果url版本不存在
        if (nonUrlVersion) {
            String vsersionMediaTypes = new MonsterMediaType(apiVersion.value()).toString();
