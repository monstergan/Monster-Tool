 ...
 * @author monster gan
 */
public class MonsterBeanMapEmitter extends ClassEmitter {
	private static final Type BEAN_MAP = TypeUtils.parseType(MonsterBeanMap.class.getName());
	...
	public MonsterBeanMapEmitter(ClassVisitor v, String className, Class type, int require) {
		super(v);
		...
		if (require != 0) {
			for (Iterator it = allProps.keySet().iterator(); it.hasNext(); ) {
				...
				if ((((require & MonsterBeanMap.REQUIRE_GETTER) != 0) && !getters.containsKey(name)) ||
					(((require & MonsterBeanMap.REQUIRE_SETTER) != 0) && !setters.containsKey(name))) {
					it.remove();
					getters.remove(name);
					setters.remove(name);
				}
