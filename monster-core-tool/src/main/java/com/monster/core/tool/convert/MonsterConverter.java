...
import com.monster.core.tool.function.CheckedFunction;
import com.monster.core.tool.utils.ClassUtil;
import com.monster.core.tool.utils.ConvertUtil;
import com.monster.core.tool.utils.ReflectUtil;
import com.monster.core.tool.utils.Unchecked;
 ...
 * <p>
 * Create by monster gan on 2023/3/13 10:49
 *
 * @author monster gan
 */
@Slf4j
@AllArgsConstructor
public class MonsterConverter implements Converter {
     ...
     * @param fieldName 目标的field名，原为 set 方法名，KteBeanCopier 里做了更改
     * @return {Object}
     */
    @Override
    @Nullable
    public Object convert(Object value, Class target, final Object fieldName) {
        ...
        try {
            TypeDescriptor targetDescriptor = MonsterConverter.getTypeDescriptor(targetClazz, (String) fieldName);
            // 1. 判断 sourceClazz 为 Map
            if (Map.class.isAssignableFrom(sourceClazz)) {
                return ConvertUtil.convert(value, targetDescriptor);
            } else {
                TypeDescriptor sourceDescriptor = MonsterConverter.getTypeDescriptor(sourceClazz, (String) fieldName);
                return ConvertUtil.convert(value, sourceDescriptor, targetDescriptor);
            }
        } catch (Throwable e) {
            log.warn("KteConverter error", e);
            return null;
        }
    }
}
