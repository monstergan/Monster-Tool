...
/**
 * 支持链式 bean，支持 map ClassLoader 跟 target 保持一致
 * <p>
 * Create on 2023/3/12 22:25
 *
 * @author monster gan
 */
public abstract class MonsterBeanCopier {

	private static final Type CONVERTER = TypeUtils.parseType("org.springframework.cglib.core.Converter");
	private static final Type BEAN_COPIER = TypeUtils.parseType(MonsterBeanCopier.class.getName());
	...
	/**
	 * The map to store {@link MonsterBeanCopier} of source type and class type for copy.
	 */
	private static final ConcurrentMap<MonsterBeanCopierKey, MonsterBeanCopier> BEAN_COPIER_MAP = new ConcurrentHashMap<>();

	public static MonsterBeanCopier create(Class source, Class target, boolean useConverter) {
		return MonsterBeanCopier.create(source, target, useConverter, false);
	}

	public static MonsterBeanCopier create(Class source, Class target, boolean useConverter, boolean nonNull) {
		MonsterBeanCopierKey copierKey = new MonsterBeanCopierKey(source, target, useConverter, nonNull);
	...
	public static class Generator extends AbstractClassGenerator {
		private static final Source SOURCE = new Source(MonsterBeanCopier.class.getName());
		...
		@Override
		public MonsterBeanCopier create(Object key) {
			return (MonsterBeanCopier) super.create(key);
		}
