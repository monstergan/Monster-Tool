 ...
 * 自定义资源文件读取，优先级最低
 * <p>
 * Create on 2023/3/13 13:46
 *
 * @author monster gan
 */
@Slf4j
public class MonsterPropertySourcePostProcessor implements BeanFactoryPostProcessor, InitializingBean, Ordered {

    private final ResourceLoader resourceLoader;
    private final List<PropertySourceLoader> propertySourceLoaders;

    public MonsterPropertySourcePostProcessor() {
    ...
    private static void loadPropertySource(String location, Resource resource,
        ...
        if (resource.exists()) {
            String name = "ktePropertySource: [" + location + "]";
            try {
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        log.info("KtePropertySourcePostProcessor process @KtePropertySource bean.");
        Map<String, Object> beansWithAnnotation = beanFactory.getBeansWithAnnotation(MonsterPropertySource.class);
        Set<Map.Entry<String, Object>> beanEntrySet = beansWithAnnotation.entrySet();
        // 没有 @YmlPropertySource 注解，跳出
        if (beanEntrySet.isEmpty()) {
            log.warn("Not found @KtePropertySource on spring bean class.");
            return;
        }
        // 组装资源
        ...
        for (Map.Entry<String, Object> entry : beanEntrySet) {
            ...
            MonsterPropertySource propertySource = AnnotationUtils.getAnnotation(beanClass, MonsterPropertySource.class);
    ...
    public void afterPropertiesSet() throws Exception {
        log.info("KtePropertySourcePostProcessor init.");
    }
