package com.monster.core.launch;

import com.monster.core.launch.constant.AppConstant;
import com.monster.core.launch.constant.NacosConstant;
...
public class MonsterApplication {
	...
	public static SpringApplicationBuilder createSpringApplicationBuilder(String appName, Class source, String... args) {
		...
		// 预设的环境
		List<String> presetProfiles = new ArrayList<>(Arrays.asList(AppConstant.PROD_CODE, AppConstant.TEST_CODE, AppConstant.DEV_CODE));
		// 交集
		...
		if (activeProfileList.isEmpty()) {
			...
			throw new IllegalStateException("同时存在环境变量:[" + StringUtils.arrayToCommaDelimitedString(activeProfiles) + "]");
		}
		String startJarPath = MonsterApplication.class.getResource("/").getPath().split("!")[0];
		...
		props.setProperty("kte.env", profile);
		props.setProperty("kte.name", appName);
		props.setProperty("kte.is-local", String.valueOf(isLocalDev()));
		props.setProperty("kte.dev-mode", profile.equals(AppConstant.PROD_CODE) ? "false" : "true");
		props.setProperty("kte.service.version", AppConstant.APPLICATION_VERSION);
		...
		defaultProperties.setProperty("spring.cloud.nacos.config.shared-configs[1].data-id", NacosConstant.sharedDataId(profile, appName));
