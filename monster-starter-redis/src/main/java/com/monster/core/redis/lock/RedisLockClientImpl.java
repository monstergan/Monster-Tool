...
import com.monster.core.redis.exception.RedisLockException;
...
public class RedisLockClientImpl implements RedisLockClient {
    ...
    @Override
    public <T> T explicitFailLock(String lockName, LockType lockType, long waitTime, long leaseTime, TimeUnit timeUnit, String message, CheckedSupplier<T> supplier) {
        try {
            boolean result = this.tryLock(lockName, lockType, waitTime, leaseTime, timeUnit);
            if (!result) {
